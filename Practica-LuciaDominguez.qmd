...
5.  Reemplazar todos los 0m\^2 por NA
    ...
    ```{r}
    # Reemplazar los apartamentos con 2 m2 por NA
...
La hipótesis nula es que no hay diferencia significativa entre las medias de todos los pares de barrios que tienen un p adj alto, con lo que podemos ver que algunos barrios como Jerónimos y Acacias si tienen diferencias pero otros muchos no las tienen.
...
12. ¿Que punto de corte sería el aconsejable?, ¿cuantos clusters aparecen? Segun la imagen ponemos el corte en 0.1 para conseguir 3 clúster.
...
Tenemos un clúster muy definido que es el 3.
...
17. Si tuvieramos un anuncio de un apartamento para 6 personas (Accommodates), con 1 baño, con un precio de 80€/noche y 3 habitaciones en el barrio de Sol, con 3 camas y un review de 80. ¿Cuantos metros cuadrados tendría? Si tu modelo necesita algúna variable adicional puedes inventartela dentro del rango de valores del dataset. ¿Como varía sus metros cuadrados con cada habitación adicional?
    ...
    ```{r}
    # Definir data frame con los valores
...
```{r}
# Seleccion las columnas númericas de mi data frame
...
new_apartment <- data.frame(Accommodates = 4, Bathrooms = 1.0, Bedrooms = 2, Beds = 4,Price = 90, Guests.Included = 2, Extra.People = 2, Review.Scores.Rating = 87, Latitude = 40.000000, Longitude = -3.00000, Square.Meters = 66.08)
...
Cogeremos los componentes que acumulan entre un 80-90% de la varianza. PCA = 6

```{r}
number_of_pca_components <- 6
knn <- 5
apart_pca <- apartment_transform[, 1:number_of_pca_components]
Apc <-pr_madrid$x[,1:number_of_pca_components]
dist <- rowSums((apart_pca - Apc)^2)   
